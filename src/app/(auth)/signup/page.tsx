...
export default function SignupPage() {
  ...
  const handleChange = (field: string, value: string) => { setForm((prev) => ({ ...prev, [field]: value })); setError(''); };
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSuccess(true);
    } catch (err) { setError(err instanceof Error ? err.message : 'Signup failed'); } finally { setLoading(false); }
  };
  ...
  const handleGoogleSignup = async () => {
    ...
    try {
      ...
      if (authError) throw authError;
    } catch (err) { setError(err instanceof Error ? err.message : 'Google signup failed'); }
  };
    ...
    return (
      <div className="flex min-h-screen items-center justify-center px-4">
        <div className="w-full max-w-sm text-center">
          ...
          <p className="mt-2 text-sm text-white/60">We sent a confirmation link to {form.email}</p>
    ...
    <div className="flex min-h-screen items-center justify-center px-4">
      <div className="w-full max-w-sm">
        <div className="mb-8 text-center">
          ...
          <p className="mt-2 text-sm text-white/60">Start building anyway</p>
        </div>
        ...
        <p className="mt-6 text-center text-sm text-white/60">
          Already have an account? <Link href="/login" className="font-medium text-brand-400 hover:text-brand-300">Sign in</Link>
        </p>
      </div>
    </div>
  );
