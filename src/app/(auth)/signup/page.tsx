...
export default function SignupPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { error: authError } = await supabase.auth.signUp({ email: form.email, password: form.password, options: { data: { full_name: form.full_name } } });
    ...
    return (
      <div className="relative flex min-h-screen items-center justify-center px-4 pt-20">
        ...
        <div className="relative w-full max-w-sm text-center">
          ...
          <p className="mt-2 text-sm text-white/50">We sent a confirmation link to {form.email}</p>
          <Link href="/login" className="mt-4 inline-block text-sm text-brand-400 hover:text-brand-300">Back to Login</Link>
        </div>
      </div>
    );
  }
    ...
    <div className="relative flex min-h-screen items-center justify-center px-4 pt-20">
      ...
      <div className="relative w-full max-w-sm">
        <div className="mb-8 text-center">
          ...
          <h1 className="mt-6 text-2xl font-bold font-[family-name:var(--font-display)]">Create an account</h1>
          <p className="mt-2 text-sm text-white/50">Start building anyway</p>
        </div>
        ...
        <div className="my-6 flex items-center gap-3"><div className="h-px flex-1 bg-white/10" /><span className="text-xs text-white/40">or</span><div className="h-px flex-1 bg-white/10" /></div>
        ...
        <p className="mt-6 text-center text-sm text-white/50">Already have an account? <Link href="/login" className="font-medium text-brand-400 hover:text-brand-300">Sign in</Link></p>
      </div>
    </div>
  );
}
