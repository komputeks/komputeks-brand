...
export function SubscribeForm({ source = 'landing' }: { source?: string }) {
  ...
  if (success) return <div className="flex items-center gap-2 text-green-400"><CheckCircle className="h-5 w-5" /><span className="font-medium">You&apos;re in! We&apos;ll keep you posted.</span></div>;
  return <form onSubmit={handleSubmit} className="flex flex-col gap-3 sm:flex-row"><div className="flex-1"><Input type="email" placeholder="Enter your email" value={email} onChange={(e) => setEmail(e.target.value)} error={error} /></div><Button type="submit" loading={loading} size="lg"><Mail className="mr-2 h-4 w-4" />Subscribe</Button></form>;
}