...
export function Card({ className, hover = false, children, ...props }: CardProps) {
  return <div className={cn('bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl', hover && 'hover:bg-white/10 transition-all duration-300', className)} {...props}>{children}</div>;
}
